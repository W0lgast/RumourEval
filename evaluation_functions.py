...
def evaluation_function_stance_branchLSTM_RumEv(params):
    ...
    y_pred, _ = LSTM_model_stance(x_train, y_train_cat, x_test, params, eval=True)
    # get tree labels
    #    pad ids first
    #%%
    fids_test = []
...
def evaluation_function_veracity_branchLSTM_RumEv(params):
    ...
    y_pred, confidence = LSTM_model_veracity(
        x_train, y_train, x_test, params, eval=True
    )
    # get tree labels

    trees, tree_prediction, tree_confidence = branch2treelabels_test(
        ids_test, y_pred, confidence
    )
