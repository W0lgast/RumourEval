...
def calculate_a_score(truth_values, submission):
    ...
    for reference_id in truth_values.keys():
        if reference_id in submission.keys():
            ...
            if observed in [8, 33, 85, 133, 200]:
                print(submission[reference_id])
...
def calculate_b_score(truth_values, submission):
    ...
    for reference_id in truth_values.keys():
        if reference_id in submission.keys():
            observed += 1
            if observed in [6, 8, 15, 17, 21]:
                print(submission[reference_id][0])
            y_true.append(truth_values[reference_id])
            ...
            try:
                ...
                print(
                    "   Each entry should be a list of two values - [veracity, confidence]"
                )
                print(
                    '   veracity is one of "true" or "false"; confidence is a float from 0..1.'
                )
                print(
                    "   This entry was:",
            ...
            if yhat == truth_values[reference_id] and (
                yhat == "true" or yhat == "false"
            ):
                correct += 1.0
...
if "subtaskaenglish" in truth_values and len(truth_values["subtaskaenglish"]) > 0:
    ...
    if "subtaskaenglish" in submission and len(submission["subtaskaenglish"]) > 0:
        correct, total, englishamacro = calculate_a_score(
            truth_values["subtaskaenglish"], submission["subtaskaenglish"]
        )
...
if "subtaskbenglish" in truth_values and len(truth_values["subtaskbenglish"]) > 0:
    ...
    if "subtaskbenglish" in submission and len(submission["subtaskbenglish"]) > 0:
        correct, total, sumerrors, lenerrors, englishbmacro = calculate_b_score(
            truth_values["subtaskbenglish"], submission["subtaskbenglish"]
        )
        ...
        print(
            "Task B (veracity), English, accuracy:",
            englishbscore,
            "RMSE:",
            englishbrmse,
        )
...
if "subtaskadanish" in truth_values and len(truth_values["subtaskadanish"]) > 0:
    ...
    if "subtaskadanish" in submission and len(submission["subtaskadanish"]) > 0:
        correct, total, danishamacro = calculate_a_score(
            truth_values["subtaskadanish"], submission["subtaskadanish"]
        )
...
if "subtaskbdanish" in truth_values and len(truth_values["subtaskbdanish"]) > 0:
    ...
    if "subtaskbdanish" in submission and len(submission["subtaskbdanish"]) > 0:
        correct, total, sumerrors, lenerrors, danishbmacro = calculate_b_score(
            truth_values["subtaskbdanish"], submission["subtaskbdanish"]
        )
        ...
        blenerrors += lenerrors
        print(
            "Task B (veracity), Danish, accuracy:", danishbscore, "RMSE:", danishbrmse
        )
...
if "subtaskarussian" in truth_values and len(truth_values["subtaskarussian"]) > 0:
    ...
    if "subtaskarussian" in submission and len(submission["subtaskarussian"]) > 0:
        correct, total, russianamacro = calculate_a_score(
            truth_values["subtaskarussian"], submission["subtaskarussian"]
        )
...
if "subtaskbrussian" in truth_values and len(truth_values["subtaskbrussian"]) > 0:
    ...
    if "subtaskbrussian" in submission and len(submission["subtaskbrussian"]) > 0:
        correct, total, sumerrors, lenerrors, russianbmacro = calculate_b_score(
            truth_values["subtaskbrussian"], submission["subtaskbrussian"]
        )
        ...
        blenerrors += lenerrors
        print(
            "Task B (veracity), Russian, accuracy:",
            russianbscore,
            "RMSE:",
            russianbrmse,
        )
...
print(
    "Task A (SDQC), MacroF (averaged over " + str(taskalangnum) + " languages):",
    macroa / taskalangnum,
)
...
print(
    "Task B (veracity), MacroF (averaged over " + str(taskblangnum) + " languages):",
    macrob / taskblangnum,
)
